...
class App(tk.Tk):
    ...
    def interface(self):
        ...
        # Chat msg screen
        self.chat_scrolledtxt = scrolledtext.ScrolledText(self, width=58, height=21, bg='white', relief=SOLID, bd=2, font=f, wrap=WORD, state='disabled')
    ...
    def receive_msg(self):
            while self.running:
                try:
                    ...
                    print(message)
                    self.chat_scrolledtxt.configure(state='normal')
                    self.chat_scrolledtxt.insert(tk.END, message + '\n')
                    self.chat_scrolledtxt.configure(state='disabled')
